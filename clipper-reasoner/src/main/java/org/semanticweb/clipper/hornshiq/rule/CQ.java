...
public class CQ {
    ...
    public Set<Variable> getDistinguishedVariables() {
        return this.head.getTerms().stream().filter(Term::isVariable).map(x -> x.asVariable()).collect(Collectors.toSet());
    }
    ...
    public Set<Variable> getNonDistinguishedVars() {
        ...
        Set<Variable> distinguishedVars = this.getDistinguishedVariables();
        for (Atom atom : body) {
